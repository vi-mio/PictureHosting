<template>
  <div>
    ...
    <el-row>
      <el-row class="cdn-input-row">
        <el-input v-model="newCdnBaseUrl" placeholder="输入新的 CDN 站源加速。" style="width: 50%;"></el-input>
        <el-button type="primary" @click="confirmCdnBaseUrl">切换 CDN 源站</el-button>
      </el-row>

    </el-row>
    <el-alert title="默认 CDN 站源：jsd.onmicrosoft.cn ; 可用的 CDN 推荐站源：testingcf.jsdelivr.net、gcore.jsdelivr.net、quantil.jsdelivr.net、fastly.jsdelivr.net、jsd.cdn.zzko.cn、jsdelivr.b-cdn.net 。" type="success"  :closable="false"></el-alert>
    <el-alert title="只显示<img>标签支持的 apng,avif,bmp,gif,ico,cur,jpg,jpeg,jfif,pjpeg,pjp,png,svg,tif,tiff,webp 格式的图片。" type="warning" show-icon close-text="不再提示" v-if="hintShow1" @close="noDisplay(1)"></el-alert>
...
export default {
  ...
  methods: {
    ...
    changeRepos() {
      this.resourceShow++; // 改变级联选择器的 key 来重置其中选项内容
    ...
    */
    ...
    imgUrl(file) {
      return this.isCDN ? `https://${this.cdnBaseUrl}/gh/${this.userInfo.login}/${this.activeRepos}/${file.path}` : file.download_url;
    },
    copy(type, file) {
      // type 1 cdn link  2 Markdown
      let imgUrl = `https://${this.cdnBaseUrl}/gh/${this.userInfo.login}/${this.activeRepos}/${file.path}`;
      let copyCont = imgUrl;
      if (type == 2) {
    ...
    upload(data) {
      ...
      reader.onload = (() => {
        ...
        // 批量上传需要间隔时间，否则可能 commit 版本号冲突，返回 409 错误码，Status: 409 Conflict
    ...
    // 新增方法用于确认新的 CDN 基础 URL
    confirmCdnBaseUrl() {
      this.cdnBaseUrl = this.newCdnBaseUrl;
    }
  },
};
</script>
